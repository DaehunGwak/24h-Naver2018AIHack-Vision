...
def get_siamese_model(input_shape=(224, 224, 3), embedding_dim=2048, weight_mode=None):
    ...
    * negative(n) : query 와 다른 분류인 image
    ...
    x = Dropout(0.2)(x)
    x = BatchNormalization()(x)
    x = Dense(embedding_dim, name='output_layer', activation='sigmoid')(x)
    ...
    triplet_model.add_loss(K.mean(triplet_lossless_loss(outputs, N=embedding_dim)))
...
def triplet_lossless_loss(inputs, N=2048, beta=None, epsilon=1e-8):
    if beta is None:
        beta = N
    anchor, positive, negative = inputs
    positive_distance = K.sum(K.square(anchor - positive), axis=-1, keepdims=True)
    negative_distance = K.sum(K.square(anchor - negative), axis=-1, keepdims=True)
