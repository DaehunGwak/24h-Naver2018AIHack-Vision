...
def bind_model(model):
    ...
    def infer(queries, _):
        ...
        # PCA
        all_vec = np.vstack((query_vecs, reference_vecs))
        pca = PCA(n_components=query_vecs.shape[1], whiten=True)
        result = pca.fit_transform(all_vec)
        query_vecs = result[:query_vecs.shape[0]]
        reference_vecs = result[query_vecs.shape[0]:]
