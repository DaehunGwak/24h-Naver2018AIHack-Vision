...
def get_siamese_model(mobile_backbone_model, dense169_backbone_model, dense201_backbone_model, input_shape=(224, 224, 3), embedding_dim=2048, p=3.):
    """
    ...
    * negative(n) : query 와 다른 분류인 image
    ...
    dense169_base_out = dense169_backbone_model(input_layer_metric)
    dense201_base_out = dense201_backbone_model(input_layer_metric)

    # mobile_base_out = Dense(embedding_dim, name='mobileNet')(mobile_base_out)
    # dense169_base_out = Dense(embedding_dim, name='denseNet169')(dense169_base_out)
    # dense201_base_out = Dense(embedding_dim, name='denseNet201')(dense201_base_out)

    x = Lambda(lambda _x: _x[0] + _x[1] + _x[2], name='metric_lambda1')([mobile_base_out, dense169_base_out, dense201_base_out])
...
def add_classification_dense_model(num_classes=1383, input_shape=(224, 224, 3), weight_mode='imagenet'):
    ...
    dense201_base_model = DenseNet201(weights=weight_mode, include_top=False,
                                  input_tensor=input_layer, input_shape=input_shape)
    dense169_base_model = DenseNet169(weights=weight_mode, include_top=False,
                                   input_tensor=input_layer, input_shape=input_shape)
    mobile_base_model = MobileNet(weights=weight_mode, include_top=False,
                                      input_tensor=input_layer, input_shape=input_shape)
    
    # layer 이름 다르게 해줘야해서 추가
    for layer in dense169_base_model.layers[1:]:
        layer.name = layer.name + '_DenseNet169'

    for layer in dense201_base_model.layers[1:]:
        layer.name = layer.name + '_DenseNet201'

    for layer in mobile_base_model.layers[1:]:
        layer.name = layer.name + '_MobileNet'

    dense169_base_output = dense169_base_model.output
    dense169_base_output = GlobalAveragePooling2D()(dense169_base_output)
    dense169_base_output = Dropout(0.2)(dense169_base_output)
    dense169_base_output = BatchNormalization()(dense169_base_output)
    dense169_base_output = Dense(2048)(dense169_base_output)
    dense169_backbone_model = Model(dense169_base_model.input, dense169_base_output, name='dense169_backbone')
    dense169_base_output = Activation(activation='relu')(dense169_base_output)
    dense169_base_output = Dropout(0.2)(dense169_base_output)
    predictions_dense169 = Dense(num_classes, activation='softmax')(dense169_base_output)
    dense169_model = Model(inputs=dense169_backbone_model.input, outputs=predictions_dense169)

    dense201_base_output = dense201_base_model.output
    dense201_base_output = GlobalAveragePooling2D()(dense201_base_output)
    dense201_base_output = Dropout(0.2)(dense201_base_output)
    dense201_base_output = BatchNormalization()(dense201_base_output)
    dense201_base_output = Dense(2048)(dense201_base_output)
    dense201_backbone_model = Model(dense201_base_model.input, dense201_base_output, name='dense201_backbone')
    dense201_base_output = Activation(activation='relu')(dense201_base_output)
    dense201_base_output = Dropout(0.2)(dense201_base_output)
    predictions_dense201 = Dense(num_classes, activation='softmax')(dense201_base_output)
    dense201_model = Model(inputs=dense201_backbone_model.input, outputs=predictions_dense201)
    ...
    return mobile_backbone_model, dense169_backbone_model, dense201_backbone_model, mobile_model, dense169_model, dense201_model
...
def get_all_generator(image_gen, class_list, input_shape=(224, 224, 3), batch_size=32, path=".", train_mode='imagenet'):
    result_list = []
    generator_list = []
